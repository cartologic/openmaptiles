...
FROM (
         ...
         SELECT ST_LineMerge(ST_Collect(geometry)) AS geometry,
                transportation_name_tags(NULL::geometry, tags, name, name_en, name_ar) AS tags,
                NULL AS ref,
                'shipway' AS highway,
                shipway AS subclass,
                NULL AS brunnel,
         ...
         WHERE name <> ''
         GROUP BY name, name_en, name_ar, tags, subclass, "level", layer
         UNION ALL
...
BEGIN
    ...
    FROM (
        SELECT hl.geometry,
            hl.osm_id,
            transportation_name_tags(hl.geometry, hl.tags, hl.name, hl.name_en, hl.name_ar) AS tags,
            rm1.network_type,
            CASE
